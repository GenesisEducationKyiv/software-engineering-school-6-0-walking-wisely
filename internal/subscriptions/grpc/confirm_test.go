...
func TestConfirm_HappyPath(t *testing.T) {
	...
	resp, err := svc.ConfirmSubscription(context.Background(), &subscriptionv1.ConfirmSubscriptionRequest{Token: validToken})
	if err != nil {
...
func TestConfirm_InvalidToken(t *testing.T) {
	...
	_, err := svc.ConfirmSubscription(context.Background(), &subscriptionv1.ConfirmSubscriptionRequest{Token: "not-a-valid-token"}) // #nosec G101 -- test-only invalid token.
...
func TestConfirm_RepoErrors(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := svc.ConfirmSubscription(context.Background(), &subscriptionv1.ConfirmSubscriptionRequest{Token: validToken})
